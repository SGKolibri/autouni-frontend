...
import { Building, EnergyComparisonItem, GlobalEnergyStats } from '@/types';
...
function buildTreeMapData(stats: GlobalEnergyStats | undefined): TreeMapNode {
  if (!stats) return { name: 'Consumo', children: [{ name: 'Sem dados', value: 1 }] };
  return {
    name: 'Consumo',
    children: [
      { name: `Total: ${stats.totalKwh.toFixed(2)} kWh`, value: stats.totalKwh || 1 },
    ],
  };
}
...
const EnergyPage = () => {
  ...
  const statsPeriod: 'today' | 'week' | 'month' =
    period === 'custom' ? 'today' : period;

  const { data: stats, isLoading: isLoadingStats } = useQuery<GlobalEnergyStats>({
    queryKey: ['energy', 'stats', statsPeriod],
    queryFn: () => apiService.getGlobalEnergyStats(statsPeriod),
  });

  const { data: historyData, isLoading: isLoadingChart } = useQuery({
    queryKey: ['energy', 'history', period, level, selectedId],
    queryFn: async () => {
      const params: Record<string, string> = { period };
      if (level !== 'general') params.level = level;
      if (selectedId) params.id = selectedId;
      const res = await apiService.getEnergyHistory(params);
      return res.history.map((b) => ({ timestamp: b.bucket, value: b.totalKwh }));
    },
  });
  ...
  const { data: comparison } = useQuery<EnergyComparisonItem[]>({
    queryKey: ['energy', 'comparison', level],
    queryFn: async () => {
      const res = await apiService.getEnergyComparison({ level });
      return res.comparison.map((c) => ({ id: c.id, name: c.name, energy: c.totalKwh, trend: 0 }));
    },
  });

  const totalKwh = stats?.totalKwh ?? 0;
  const totalCost = totalKwh * 0.85;
  const peakWh = stats?.maxWh ?? 0;
  const avgWh = stats?.avgWh ?? 0;
    ...
    <Box>
        ...
        <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', sm: 'repeat(2, 1fr)', md: 'repeat(4, 1fr)' }, gap: { xs: 2, md: 3 }, mb: 3 }}>
            ...
            <CardContent>
              <Box sx={{ display: 'flex', alignItems: 'center', mb: 2 }}>
                ...
                <Box>
                  ...
                  <Typography variant="h5" fontWeight={700}>{totalKwh.toFixed(2)} kWh</Typography>
                </Box>
              </Box>
            </CardContent>
          </Card>
          <Card>
            <CardContent>
              ...
              <Typography variant="h5" fontWeight={700} color="error.main" gutterBottom>R$ {totalCost.toFixed(2)}</Typography>
              <Typography variant="caption" color="text.secondary">Tarifa: R$ 0,85/kWh</Typography>
            </CardContent>
          </Card>
          <Card>
            <CardContent>
              <Typography variant="caption" color="text.secondary" gutterBottom>Pico de Leitura</Typography>
              <Typography variant="h5" fontWeight={700} gutterBottom>{peakWh.toFixed(0)} Wh</Typography>
              <Typography variant="caption" color="text.secondary">Máximo registrado</Typography>
            </CardContent>
          </Card>
          <Card>
            <CardContent>
              <Typography variant="caption" color="text.secondary" gutterBottom>Leitura Média</Typography>
              <Typography variant="h5" fontWeight={700} gutterBottom>{avgWh.toFixed(0)} Wh</Typography>
      ...
      <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', lg: '1fr 320px' }, gap: { xs: 2, md: 3 }, mb: 3 }}>
        ...
        <Paper sx={{ p: { xs: 2, sm: 3 }, minWidth: 0 }}>
          ...
          <GaugeChart
            value={realtimeEnergy?.power ?? avgWh}
            max={peakWh ? peakWh * 1.2 : 5000}
            unit="Wh"
            label="Leitura Atual"
          />
        </Paper>
      ...
      <Paper sx={{ p: { xs: 2, sm: 3 }, overflow: 'hidden' }}>
        ...
        <TableContainer sx={{ overflowX: 'auto' }}>
          <Table sx={{ minWidth: { xs: 'auto', sm: 650 } }}>
            ...
            <TableBody>
              {comparison && comparison.length > 0 ? comparison.map((item, index) => (
